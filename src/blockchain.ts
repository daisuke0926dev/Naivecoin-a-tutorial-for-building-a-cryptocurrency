...
const findBlock = (index: number, previousHash: string, timestamp: number, data: string, difficulty: number): Block => {
    ...
    while (true) {
        ...
        if(hashMachesDifficulty(hash, difficulty)) {
...
const hasValidHash = (block: Block): boolean => {
    ...
    if (!hashMatchesBlocContent(block)) {
        console.log('invalid hash, got:' + block.hash);
        return false;
    }

    if(!hashMachesDifficulty(block.hash, block.difficulty)) {
        console.log('block difficulty not satisfied. Expeccted: ' + block.difficulty + 'got:' + block.hash);
    }
    return true;
}

const hashMatchesBlocContent = (block: Block): boolean => {
...
const isValidChain = (blockchainToValidate: Block[]): boolean => {
    const isValidGenesis = (block: Block): boolean => {
        return JSON.stringify(block) === JSON.stringify(generateNextBlock);
    };
...
const replaceChain = (newBlocks: Block[]) => {
    if (isValidChain(newBlocks) && newBlocks.length > getBlockchain().length) {
        console.log('Received blockchain is valid. Replacing current blockchain wirh received blockchain');
